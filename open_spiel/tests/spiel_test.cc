...
void SupportedGameTests() {
  RandomSimTest(*LoadGame("carcassonne"), /*num_sims=*/10);
  RandomSimTest(*LoadGame("othello"), /*num_sims=*/10);
}

void GameEqualityTests() {
  SPIEL_CHECK_TRUE(*LoadGame("tic_tac_toe") == *LoadGame("tic_tac_toe"));
  SPIEL_CHECK_FALSE(*LoadGame("tic_tac_toe") == *LoadGame("othello"));
}
...
void PolicyTest() {
  ...
  for (const std::string& game_name : {"tic_tac_toe"}) {
...
void DeserializeGameAndStateTest() {
  std::shared_ptr<const Game> game = LoadGame("tic_tac_toe");
  std::unique_ptr<State> state = game->NewInitialState();
  state->ApplyAction(0);
  state->ApplyAction(1);
  const std::string serialized_game_and_state =
      SerializeGameAndState(*game, *state);

  std::pair<std::shared_ptr<const Game>, std::unique_ptr<State>>
      game_and_state =
          open_spiel::DeserializeGameAndState(serialized_game_and_state);

  SPIEL_CHECK_EQ(game_and_state.first->ToString(), game->ToString());
  SPIEL_CHECK_EQ(game_and_state.second->ToString(), state->ToString());
  SPIEL_CHECK_EQ(
...
void LoadGameFromJsonTest() {
  ...
  // Test loading another supported game.
  {
    auto game = LoadGameFromJson(R"({"game_name": "othello"})");
    SPIEL_CHECK_EQ(game->GetType().short_name, "othello");
  }
}
...
int main(int argc, char** argv) {
  ...
  open_spiel::testing::SupportedGameTests();
  ...
  open_spiel::testing::DeserializeGameAndStateTest();
